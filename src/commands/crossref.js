...
/**
 * Full data-fetch pipeline for cross-references: fetches the raw TSK refs,
 * fetches source verse text, fetches each target ref's text, truncates.
 * Returns `{ sourceLabel, sourceText, refs, totalRefCount, translation }`
 * ready to pass into `buildCrossrefPage`. Returns null if the source verse
 * can't be fetched in the requested translation.
 *
 * Exported so non-command surfaces (e.g., the welcome card's cross-ref
 * demo button) can produce identical output to the /crossref slash command.
 */
export async function fetchCrossrefData({ bookId, bookName, chapter, verse, translation }) {
    ...
    if (crossRefResult.status === 'rejected' || !crossRefResult.value || crossRefResult.value.length === 0) {
        return { sourceLabel, sourceText, refs: [], totalRefCount: 0, translation };
    }
    ...
    return { sourceLabel, sourceText, refs, totalRefCount, translation };
}

export function buildCrossrefPage({ data, pageIdx, totalPages, disableNav = false }) {
...
export default {
    ...
    async execute(interaction, database) {
        ...
        try {
            ...
            const data = await fetchCrossrefData({ bookId, bookName, chapter, verse: verseInput, translation });
            if (!data) {
            ...
            if (data.refs.length === 0 && data.totalRefCount === 0) {
                ...
                return interaction.editReply({
                    ...
                    components: [
                        new ContainerBuilder()
                            .setAccentColor(accentColor())
                            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`## 🔗 Cross References — ${data.sourceLabel}`))
                            .addTextDisplayComponents(new TextDisplayBuilder().setContent(`**${translation.toUpperCase()}:** ${data.sourceText}`))
            ...
            if (data.refs.length === 0) {
                return interaction.editReply({
                    flags: MessageFlags.IsComponentsV2,
                    components: [new TextDisplayBuilder().setContent(
                        `Cross-references found for ${data.sourceLabel} but couldn't retrieve verse text in ${translation.toUpperCase()}.`
                    )]
                });
            }
