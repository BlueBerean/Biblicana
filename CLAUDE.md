...
- **post-v1.6.0, deployed 2026-09-06** — one-chapter books, and ESLint. `13dec7f`: **"Jude 5" means Jude 1:5**, not chapter 5 of a book that has one chapter, and it had been resolving to nothing everywhere — passive detection posted no card, and the AI's reference tools replied "that's a chapter, not a verse, try Jude 5:1", advice that cannot work. Obadiah, Philemon, 2 John and 3 John failed identically. Fixed in `parseScriptureRefs` (which covers passive detection, the context menus, the reaction handler and every AI tool taking a reference string, since they all funnel through `parseSingleVerseRef`) plus a separate `resolveSingleChapterRef` for the **ten** slash commands, which never build a reference string. Also reads `Jude 5-7` as a verse range, and uses the verse counts as a bound so "I have read Philemon 30 times" is not a citation. `7031d18`: `.eslintrc.json` had been missing `sourceType: "module"` since the ESM migration in `6bd3d78`, so all 110 files failed to **parse** and no rule had ever run — `pnpm run lint` was checking nothing while looking like it worked. Restoring it surfaced 14 minor findings, all fixed or documented. Neither commit changed a command's option definitions, so both needed a restart and **no `deployg`**.
