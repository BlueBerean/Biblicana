...
- **`readOnly: true` does nothing.** Every wrapper in `studyHelper.js` except `bsbFootnotesWrapper` opens its SQLite with `open({ ..., readOnly: true })`, but the `sqlite` package reads only `mode`. They actually open READ-WRITE with CREATE, so a missing data file is not an error at startup — it is silently created empty, and the first query fails with "no such table". Use `mode: sqlite3.OPEN_READONLY` in anything new; `tests/bsbFootnotes.test.js` pins it for the one wrapper that does.
