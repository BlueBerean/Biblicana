...
- **Package manager**: **pnpm 10 everywhere**, from the tracked `pnpm-lock.yaml` (`pnpm install --frozen-lockfile`). Prod used npm with an untracked, stale (v1.4.0) `package-lock.json` until the 2026-09-25 migration, so what ran in prod was never exactly what was tested; that is over.
...
The old droplet `biblicana-bot-prod` is kept, **bot stopped**, as the rollback until it is snapshotted and destroyed (see `docs/ops/host-migration-2026-09.md` step 10). Rollback: `sudo -iu biblicana pm2 stop index` here, `pm2 start index` there. Never run both: two processes with the prod token both answer every event.
